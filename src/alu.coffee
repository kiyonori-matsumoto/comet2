class Alu
  FUNCTIONS =
    ...
    cpa:  (a, b) -> [a - b, false]
    cpl:  (a, b) -> [a - b, false]
    sla:  (a, b) -> [a << b, check_bit_l(a, b)]
    sra:  (a, b) -> [a >> b, check_bit_r(a, b)]
    sll:  (a, b) -> [a << b, check_bit_l(a, b)]
    srl:  (a, b) -> [a >> b, check_bit_r(a, b)]
  ...
  exec: (method,  a, b) ->
    ...
    [a, b] = to_arithmetic([a, b]) if method.type == 'ARITHMETIC'
  ...
  check_bit_l = (a, b) ->
    if b == 0 then false else ((a << (b - 1)) & 0x8000) != 0

  check_bit_r = (a, b) ->
    if b == 0 then false else ((a >> (b - 1)) & 0x1) != 0
